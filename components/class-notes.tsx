...
export function ClassNotes() {
  ...
  const filteredNotes =
    activeTab === "all"
      ? notes
      : notes.filter((note) => note.className === activeTab);
          ...
          {isAddingNote && (
            <div className="mb-6 p-4 border border-blue-100 rounded-lg bg-blue-50">
              ...
              <select
                ...
                onChange={(e) =>
                  setNewNote({ ...newNote, className: e.target.value })
                }
              >
                {classes.map((cls) => (
              ...
              {filteredNotes.length > 0 ? (
                <div className="space-y-4">
                  {filteredNotes.map((note) => (
                    <div
                      key={note.id}
                      className="p-4 rounded-lg bg-white border border-blue-100 shadow-sm"
                    >
