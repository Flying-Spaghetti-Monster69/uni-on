...
import { PlusCircle, BookOpen, Edit, Trash2, Save, BarChart, FileText, Award } from "lucide-react"
...
export function AcademicTracker() {
  ...
  const [notes, setNotes] = useState<Note[]>([
    {
      ...
      content: "Quick sort, merge sort, bubble sort. Time complexity analysis and implementation details.",
      courseId: "cs101",
    ...
    },
    {
      ...
      content: "Addition, subtraction, multiplication of matrices. Determinants and inverses.",
      courseId: "math202",
    ...
    },
    {
      ...
      content: "Classical and operant conditioning. Pavlov's experiments and Skinner's contributions.",
      courseId: "psych110",
  ...
  const getAssignmentsByCourse = (courseId: string) => {
    return courseId === "all" ? assignments : assignments.filter((assignment) => assignment.courseId === courseId)
  }

  const getNotesByCourse = (courseId: string) => {
    return courseId === "all" ? notes : notes.filter((note) => note.courseId === courseId)
  }
  ...
  const calculateCourseAverage = (courseId: string) => {
    ...
    const totalScore = courseAssignments.reduce((sum, assignment) => sum + assignment.score, 0)
    ...
    <Card className="border-blue-200 shadow-md bg-white/90 backdrop-blur-sm">
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <CardTitle className="text-blue-700 text-lg">Academic Tracker</CardTitle>
            ...
            <Dialog>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  ...
                  <DialogDescription>Choose what you want to add to your academic tracker.</DialogDescription>
                </DialogHeader>
      ...
      <CardContent>
        {isAddingAssignment && (
          <div className="mb-6 p-4 border border-blue-100 rounded-lg bg-blue-50">
            <h3 className="font-medium text-blue-700 mb-3">Add New Assignment Score</h3>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-blue-700 mb-1 block">Your Score</label>
                  <Input
                    type="number"
                    className="border-blue-200"
                ...
                <div>
                  <label className="text-sm text-blue-700 mb-1 block">Max Score</label>
                  <Input
                    type="number"
                    className="border-blue-200"
              ...
              <div className="flex space-x-2">
                <Button onClick={addAssignment} className="flex-1 bg-blue-600 hover:bg-blue-700">
        ...
        {isAddingNote && (
          <div className="mb-6 p-4 border border-teal-100 rounded-lg bg-teal-50">
            <h3 className="font-medium text-teal-700 mb-3">Add New Class Note</h3>
            ...
            <div className="space-y-3">
              ...
              <div className="flex space-x-2">
                <Button onClick={addNote} className="flex-1 bg-teal-600 hover:bg-teal-700">
                  <Save className="mr-2 h-4 w-4" /> Save
                </Button>
                <Button
                  variant="outline"
                ...
                getNotesByCourse(selectedCourse).map((note) => {
                  ...
                  return (
                    <Accordion type="single" collapsible key={note.id}>
                      <AccordionItem value={note.id} className="border border-teal-100 rounded-lg bg-teal-50 px-3">
                        <AccordionTrigger className="py-2 hover:no-underline">
                          <div className="flex flex-col items-start text-left">
                            <h3 className="font-medium text-teal-700">{note.title}</h3>
          ...
          <TabsContent value="assignments" className="mt-0">
              ...
              <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-100 flex justify-between items-center">
                <div>
                  <h3 className="font-medium text-blue-700">
                    {getCourseById(selectedCourse)?.code}: {getCourseById(selectedCourse)?.name}
                  </h3>
                  <p className="text-sm text-gray-600">Course Average</p>
                </div>
                <div className={`text-2xl font-bold ${getScoreColor(calculateCourseAverage(selectedCourse))}`}>
            ...
            <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
              {getAssignmentsByCourse(selectedCourse).length > 0 ? (
                getAssignmentsByCourse(selectedCourse).map((assignment) => {
                  ...
                  return (
                    <div key={assignment.id} className="p-3 rounded-lg bg-blue-50 border border-blue-100">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h3 className="font-medium text-blue-700">{assignment.title}</h3>
                          <div className="flex items-center text-xs text-gray-500">
                            <BookOpen className="h-3 w-3 mr-1" />
                            {course?.code} • {assignment.date.toLocaleDateString()}
                          </div>
                        </div>
                      ...
                      <div className="mb-2">
                        <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                          <div
                            className={`h-full ${getScoreColor(percentage).replace("text-", "bg-")}`}
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  >
                    <PlusCircle className="mr-2 h-4 w-4" /> Add your first assignment
                  </Button>
                </div>
              )}
            </div>
            ...
            <div className="mt-3 text-center">
              <Button variant="outline" size="sm" className="text-blue-700 border-blue-200">
                <BarChart className="mr-1 h-3.5 w-3.5" /> View Performance Analytics
              </Button>
            </div>
          </TabsContent>
        </Tabs>
